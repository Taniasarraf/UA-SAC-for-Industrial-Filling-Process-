...
class FillingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.switch_weight is not None and self.switch_step is not None:
            idx = max(15, self.switch_step + self.inflation_window)
        else:
            idx = 15  

        if len(self.ep_nees) > idx:
            info['mean_nis']  = float(np.mean(self.ep_nis[idx:]))
            info['mean_nees'] = float(np.mean(self.ep_nees[idx:]))
        else:
            info['mean_nis']  = float(np.mean(self.ep_nis[15:])) if len(self.ep_nis) > 15 else float(np.mean(self.ep_nis)) if len(self.ep_nis) > 0 else 0.0
            info['mean_nees'] = float(np.mean(self.ep_nees[15:])) if len(self.ep_nees) > 15 else float(np.mean(self.ep_nees)) if len(self.ep_nees) > 0 else 0.0
