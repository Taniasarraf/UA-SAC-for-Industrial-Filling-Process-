...
Q_nominal = np.diag([1.0, 50.0, 1.0])   
...
print(f"Executing Monte Carlo Simulation Engine ({num_trials} Trials)...")

for trial in range(num_trials):
    
    true_alpha = np.random.uniform(0.7, 1.5)
    x_hat = np.array([[0.0], [0.0], [1.0]])
    P = np.diag([10.0, 100.0, 10.0])         
    ...
    simulated_agent_switch_threshold = np.random.uniform(430.0, 460.0)
    ...
    for k in range(n):
        ...
        u_for_kf = -1.0 if is_fine else 1.0
        ...
        rate = (fine_rate if is_fine else coarse_rate) + (true_alpha * sigma_proc * np.random.randn())
        ...
        all_nees[trial, k] = float(err_x.T @ np.linalg.inv(P) @ err_x)
...
plt.rcParams.update({'font.size': 8, 'font.family': 'serif', 'axes.titlesize': 9})
fig, axs = plt.subplots(2, 2, figsize=(8.5, 6)) 


ax1 = axs[0, 0]
v_len = len(vis_data['time'])
ax1.plot(vis_data['time'], vis_data['meas_w'][:v_len], color='gainsboro', linewidth=0.8, label='Measured')
ax1.plot(vis_data['time'], vis_data['true_w'][:v_len], 'r--', linewidth=1.2, label='True Weight')
ax1.plot(vis_data['time'], vis_data['kf_w'][:v_len], 'b', linewidth=1.0, label='KF Estimate')
ax1.set_title('Weight State Tracking Profile')
...
ax2_left = axs[0, 1]
line1 = ax2_left.plot(time_steps, rmse_weight, 'g', linewidth=1.2, label='Mass RMSE')
ax2_left.set_xlabel('Session Time (s)')
ax2_left.set_ylabel('Mass Error Resolution (g)')
ax2_left.grid(True, alpha=0.2)

ax2_right = ax2_left.twinx()
line2 = ax2_right.plot(time_steps, rmse_flow, 'c', linewidth=0.9, label='Flow Rate RMSE')
ax2_right.set_ylabel('Flow Error Resolution (g/s)')

lines = line1 + line2
labels = [l.get_label() for l in lines]
ax2_left.legend(lines, labels, loc='upper right', fontsize=7, framealpha=0.8)
ax2_left.set_title('Ensemble Tracking RMSE Profile')


ax3 = axs[1, 0]
ax3.plot(time_steps, mean_nis, 'm', linewidth=1.0, label='Mean NIS')
...
ax3.set_title('Measurement Consistency (NIS)')
ax3.set_xlabel('Session Time (s)')
ax3.set_ylabel('NIS Value')
ax3.legend(loc='upper right', fontsize=7, framealpha=0.8)
ax3.grid(True, alpha=0.2)


ax4 = axs[1, 1]
ax4.plot(time_steps, mean_nees, 'darkblue', linewidth=1.0, label='Mean NEES')
...
ax4.set_yscale('log')
ax4.set_title('State Consistency Bound (NEES)')
ax4.set_xlabel('Session Time (s)')
ax4.set_ylabel('NEES Metric (Log)')
ax4.legend(loc='upper right', fontsize=7, framealpha=0.8)
ax4.grid(True, which="both", alpha=0.2)

plt.tight_layout()
plt.savefig("kf_monte_carlo_validation.png", dpi=300, bbox_inches='tight')
print("Journal friendly 2x2 multi-panel figure exported successfully.")
plt.show()


success_count = sum(740.0 <= w <= 760.0 for w in final_weights)

print(f"Total Evaluated Test Runs: {num_trials}")
print(f"Target Threshold Success Rate [740g - 760g]: {success_count / num_trials * 100:.1f}%")
print(f"Average Final Dispatched Material Weight: {np.mean(final_weights):.2f}g")