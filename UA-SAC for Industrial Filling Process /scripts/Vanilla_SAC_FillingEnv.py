...
class FillingEnvPure(gym.Env):
    ...
    def _get_obs(self):
        ...
        state = np.array([self.raw_weight_estimate, self.raw_flow_estimate, dist_to_go])
    ...
    def step(self, action):
        ...
        if not self.is_fine and self.true_weight >= (self.target_weight - self.mu_residual):
            ...
            info = {"true_weight": self.true_weight, "status": "Overflow", "mean_nis": 0.0, "mean_nees": 0.0}
        ...
        else: time_cost = -5.0 if self.raw_weight_estimate > 450.0 else -0.01
