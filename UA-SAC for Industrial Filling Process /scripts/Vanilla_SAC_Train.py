...
from Vanilla_SAC_FillingEnv import FillingEnvPure
...
class GuidedSampler:
    ...
    def act(self, current_raw_weight):
        if current_raw_weight < self.target_switch_weight:
...
def train():
    ...
    output_dir = "Outputs_Vanilla_Baseline"
    ...
    for ep in range(1500):
        ...
        if (ep + 1) % 100 == 0:
            ...
            print(f"--- Ep {ep+1:04d} | Success: {avg_success:.2f} | Error: {avg_error:.2f}g ---")
    ...
    }, os.path.join(output_dir, "vanilla_final_data.pth"))
    print(f"Baseline binary analytics file successfully saved.")

    torch.save(actor.state_dict(), os.path.join(output_dir, "vanilla_actor_final.pth"))
