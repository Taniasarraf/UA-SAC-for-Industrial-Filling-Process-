...
def plot_learning_results(history, title, output_dir, rolling_window=100):
    ...
    if 'rewards' in history and len(history['rewards']) > 0:
        ...
        plt.title("UA-TD3 Episodic Cumulative Reward Convergence")
        plt.xlabel("Episode")
        plt.ylabel("Cumulative Reward")
        plt.grid(True, alpha=0.3)
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "uatd3_avg_reward.png"), dpi=200)
        plt.close()
    ...
    plt.title(f"UA-TD3 Operational Success Rate Tracking (Rolling {window})")
    plt.xlabel("Episode")
    plt.ylabel("Success Rate Boundary")
    plt.ylim([0, 1.05])
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "uatd3_success_rate.png"), dpi=200)
    plt.close()
    ...
    if valid_switch_eps:
        ...
        plt.title("UA-TD3 Coarse-to-Fine Valve Cutoff Adaptation Window")
        plt.ylabel("Weight at Cutoff Point (g)")
    else:
        plt.title("UA-TD3 Switch Point Adaptation (No Valid Sequences)")
    plt.xlabel("Episode")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "uatd3_switch_points.png"), dpi=200)
    plt.close()
    ...
    if 'mean_nis' in history and any(v > 0 for v in history['mean_nis']):
        ...
        plt.title("UA-TD3 Normalized Innovation Squared (NIS) Health Metric")
        plt.xlabel("Episode")
        plt.ylabel("Innovation Scale (NIS)")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "uatd3_nis.png"), dpi=200)
        plt.close()
    ...
    if 'mean_nees' in history and any(v > 0 for v in history['mean_nees']):
        ...
        plt.title("UA-TD3 Normalized Estimation Error Squared (NEES) Verification")
        plt.xlabel("Episode")
        plt.ylabel("State Covariance Error Vector (NEES)")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "uatd3_nees.png"), dpi=200)
        plt.close()
    ...
    if history['final_weights']:
        ...
        plt.title("UA-TD3 Completed Episode Mass Yield Boundary Map")
        plt.xlabel("Measured Yield Mass (g)")
        plt.ylabel("Frequency Count")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "uatd3_weight_dist.png"), dpi=200)
        plt.close()
    ...
    if 'errors' in history and len(history['errors']) >= window:
        ...
        plt.title(f"UA-TD3 Mean Absolute Mass Tracking Deviation Profile (Rolling {window})")
        plt.xlabel("Episode")
        plt.ylabel("Mass Tracking Error Deviation (g)")
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "uatd3_mae.png"), dpi=200)
        plt.close()
...
def plot_loss_convergence(actor_losses, critic_losses, output_dir):
    ...
    plt.plot(critic_losses, color='darkred', alpha=0.5, linewidth=0.8, label='Critic Net MSE Loss')
    plt.plot(actor_losses, color='darkcyan', alpha=0.5, linewidth=0.8, label='Actor Policy Loss')
    ...
    if len(actor_losses) > w:
        ...
        plt.plot(range(w-1, len(critic_losses)), c_smooth, color='red', linewidth=1.8, label='Smoothed Critic Loss')
        plt.plot(range(w-1, len(actor_losses)), a_smooth, color='teal', linewidth=1.8, label='Smoothed Actor Loss')
        
    plt.title("UA-TD3 Network Parameter Convergence History")
    plt.xlabel("Optimization Weight Steps")
    plt.ylabel("Calculated Loss Bounds")
    plt.yscale('symlog') 
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "uatd3_loss_curves.png"), dpi=200)
    plt.close()
...
def save_summary_json(history, filename):
    ...
    errors_arr = np.array(history['errors']) if ('errors' in history and history['errors']) else np.array([])
    mae_val = float(np.mean(errors_arr)) if len(errors_arr) > 0 else 0.0
    mse_val = float(np.mean(errors_arr ** 2)) if len(errors_arr) > 0 else 0.0
    rmse_val = float(np.sqrt(mse_val)) if len(errors_arr) > 0 else 0.0
    ...
    summary = {
        ...
        "mean_absolute_error_g": mae_val,
        "mean_squared_error_g2": mse_val,
        "root_mean_squared_error_g": rmse_val,
    ...
    print(f"UA-TD3 Summary data saved to JSON file: {filename}")

def plot_td3_q_landscape(critic, save_path, device, obs_scale):
    weights = np.linspace(0, 800, 100)
    test_states = [[w, 20.0, 1.0, 0.1, 0.1, 0.1, max(0, 750-w)] for w in weights]
    test_states_scaled = np.array(test_states, dtype=np.float32) / obs_scale
    test_states_t = torch.FloatTensor(test_states_scaled).to(device)
    dummy_actions = torch.full((len(weights), 1), -1.0).to(device) # Fine valve action
    
    with torch.no_grad():
        
        q_values = critic.Q1(test_states_t, dummy_actions).cpu().numpy()

    plt.figure(figsize=(9, 5))
    plt.plot(weights, q_values, label="Evaluated Q1 Value State", color='darkred', linewidth=2)
    plt.axvline(x=750, color='r', linestyle='--', label='Target Weight Boundary')
    plt.title("UA-TD3 Q1-Value Landscape Profile View")
    plt.xlabel("Digital Twin Calculated Weight (g)")
    plt.ylabel("Action-Value Return Magnitude")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
