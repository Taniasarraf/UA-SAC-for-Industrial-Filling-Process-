...
class FillingEnv(gym.Env):
    """
    Uncertainty-Aware Industrial Filling Environment (UA Framework)
    Integrates Kalman Filter (KF) state estimation with observation spaces
    accounting for sensor quantization, noise, and valve switching dynamics.
    """
    def __init__(self):
        ...
        #  residual material dynamics parameters
        ...
        # Observation Space bounds: 7-D Vector [x_hat(3), P_diag(3), dist_to_go(1)]
        low = np.array([0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 0.0], dtype=np.float32)
        high = np.array([800.0, 700.0, 2.0, 1000.0, 1000.0, 1000.0, 800.0], dtype=np.float32)
        self.observation_space = spaces.Box(low=low, high=high, dtype=np.float32)
        
        # Action Space: Continuous [-1.0, 1.0] -> Thresholded at 0.0 for Fine (-1) / Coarse (+1)
    ...
    def reset(self, seed=None, options=None):
        ...
        # Randomize true process disturbance parameter per episode
        self.true_alpha = np.random.uniform(0.7, 1.5) 
        
        # Initial KF state estimate: [weight, rate, alpha]
        self.x_hat = np.array([[0.0], [0.0], [1.0]]) 
        
        # Initial KF error covariance
    ...
    def step(self, action):
        ...
        # Covariance inflation handling for initial transient and switching phases
        ...
        # Physical system step with process noise & initial startup surge
        ...
        # Ensure positive semi-definiteness of diagonal covariance
        for i in range(3):
            self.P[i, i] = max(1e-6, self.P[i, i])

        # Track Normalized Estimation Error Squared (NEES)
        ...
        # Check early termination due to premature overflow in coarse phase
        ...
        if self.is_fine:
            ...
            time_cost = -5.0 if self.x_hat[0, 0] > 450.0 else -0.01
